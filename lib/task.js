...
/**
  ...
  Task writers should not override this.
*/
Task.prototype.writeFile = function (config, content, filepath) {
  this.emitter.emit('writeFile', config, content, filepath);
  if (config.flags && config.flags.dryRun) {

  } else {
    return this.file.write(filepath, content, config);
  }
};
...
Task.prototype.run = function (config) {
  ...
  var method = this[this.method||'processFiles'];
